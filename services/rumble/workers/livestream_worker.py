...
class RumbleLivestreamWorker:
    ...
    def __init__(self, ctx, jobs: JobRegistry):
        ...
        # REQUIRED: known livestream chat channel ID (e.g. 424574510)
        self.channel_id: Optional[str] = getattr(ctx, "rumble_chat_channel_id", None)
    ...
    async def run(self):
        ...
        if not self.channel_id:
            log.error(f"[{self.ctx.creator_id}] Missing rumble_chat_channel_id in creator config")
            return

        while True:
            try:
                # If chat task died, recreate it
                if not self.chat_task or self.chat_task.done():
                    if self.chat_task and self.chat_task.done():
                        # log task exception if any
                        exc = self.chat_task.exception() if not self.chat_task.cancelled() else None
                        if exc:
                            log.error(f"[{self.ctx.creator_id}] Chat task exited with error: {exc}")
    ...
    async def _start_chat(self):
        log.info(f"[{self.ctx.creator_id}] Attaching bot to Rumble chat channel {self.channel_id}")
    ...
    async def _stop_chat(self):
        if self.chat_task:
            log.info(f"[{self.ctx.creator_id}] Detaching bot from Rumble chat")
